...
#include "../../../assets/ui/obs_theme.hpp"
...
struct Receiver final {
  ...
  std::atomic<bool> propertiesRefreshPending{false};
  ...
  void updateState(
    ...
    if (connectionChanged || sampleRateChanged || channelsChanged) {
      propertiesRefreshPending.store(true, std::memory_order_release);
    }
  }
...
void sourceTick(void* data, float) {
  auto* receiver = static_cast<Receiver*>(data);
  if (
    receiver != nullptr &&
    receiver->propertiesRefreshPending.exchange(false, std::memory_order_acq_rel)) {
    obs_source_update_properties(receiver->source);
  }
}
...
obs_properties_t* sourceProperties(void* data) {
  ...
  dhre::ui::addHeader(properties, "dhrelink_header", "dhreLink");
  dhre::ui::addStatus(properties, "dhrelink_status", isConnected
    ? "● Conectado · Recibiendo audio desde el DAW."
    : "○ Esperando la señal del DAW…", isConnected);
  ...
  dhre::ui::addSection(properties, "dhrelink_format", format,
    dhre::ui::kWhiteHex, dhre::ui::kBodyPixels, dhre::ui::kStrongWeight);
  dhre::ui::addSection(properties, "dhrelink_route", "DAW   →   dhreLink   →   OBS");
  dhre::ui::addSection(
    properties,
    "dhrelink_help",
    "La conexión es automática. Mantén una sola instancia de dhreLink Sender activa.");
  dhre::ui::addCredit(properties, "dhrelink_credit");
  return properties;
}

const char* darkIcon(void*) {
  return obs_module_file("dhreian-mark-purple-512.png");
}
