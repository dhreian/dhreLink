...
using ObsPropertyDescription = const char* (*)(void*);
using ObsPropertyTextInfoType = int (*)(void*);
using ObsPropertyTextInfoWordWrap = bool (*)(void*);
...
int wmain(int argc, wchar_t** argv) {
  ...
  const auto propertyDescription =
    loadFunction<ObsPropertyDescription>(obs, "obs_property_description");
  const auto propertyTextInfoType =
    loadFunction<ObsPropertyTextInfoType>(obs, "obs_property_text_info_type");
  const auto propertyTextInfoWordWrap = loadFunction<ObsPropertyTextInfoWordWrap>(
    obs, "obs_property_text_info_word_wrap");
  ...
  if (
    ...
    propertiesDestroy == nullptr || propertyDescription == nullptr ||
    propertyTextInfoType == nullptr || propertyTextInfoWordWrap == nullptr ||
    getDarkIcon == nullptr || getLightIcon == nullptr || freeMemory == nullptr) {
  ...
  auto* statusProperty =
    properties != nullptr ? propertiesGet(properties, "dhrelink_status") : nullptr;
  auto* formatProperty =
    properties != nullptr ? propertiesGet(properties, "dhrelink_format") : nullptr;
  const auto* statusDescription =
    statusProperty != nullptr ? propertyDescription(statusProperty) : nullptr;
  const auto* formatDescription =
    formatProperty != nullptr ? propertyDescription(formatProperty) : nullptr;
  const auto propertiesAreComplete =
    properties != nullptr &&
    statusProperty != nullptr &&
    propertiesGet(properties, "dhrelink_header") != nullptr &&
    propertiesGet(properties, "dhrelink_route") != nullptr &&
    formatProperty != nullptr &&
    propertiesGet(properties, "dhrelink_help") != nullptr &&
    propertiesGet(properties, "dhrelink_credit") != nullptr &&
    statusDescription != nullptr &&
    std::string_view(statusDescription).find("○ Esperando la señal del DAW…") != std::string_view::npos &&
    std::string_view(statusDescription).find("#874DFA") != std::string_view::npos &&
    formatDescription != nullptr &&
    std::string_view(formatDescription).find("Formato: Esperando audio") != std::string_view::npos &&
    propertyTextInfoType(statusProperty) == 0 &&
    propertyTextInfoWordWrap(statusProperty);
