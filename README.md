...
El estado de la adaptacion a macOS y los requisitos para un instalador sin
certificado de pago estan en [docs/MACOS.md](docs/MACOS.md).

dhreLink lleva el audio ya procesado de un DAW a OBS Studio en Windows y macOS, sin
...
ni procesos en segundo plano. Quintessential y Montserrat están integradas en
el propio VST3 y solo se cargan mientras su interfaz está abierta. El instalador se
...
Con la carpeta VST3 predeterminada, el instalador coloca los componentes en
las ubicaciones globales recomendadas:

```text
C:\Program Files\Common Files\VST3\dhreian\dhreLink Sender.vst3
C:\ProgramData\obs-studio\plugins\dhreLink\bin\64bit\dhreLink.dll
```

El VST3 se agrupa bajo la carpeta del fabricante dentro de la carpeta que
selecciones. El plugin de OBS permanece directamente bajo la carpeta de plugins
de dhreLink, siguiendo la estructura que OBS espera descubrir.
...
El VST3 y las propiedades de OBS comparten las reglas visuales de dhreView.
La paleta, tipografía y medidas se mantienen en [assets/ui/design-tokens.json](assets/ui/design-tokens.json);
[assets/ui/README.md](assets/ui/README.md) describe cómo sincronizar y comprobar ambos proyectos.
...
Para próximas versiones, Windows toma su versión de `CMakeLists.txt` y macOS de `packaging/macos/VERSION`; el mismo `main` contiene ambas plataformas. La [guía de macOS](docs/MACOS.md) detalla las dependencias y las limitaciones de la beta. Antes de anunciar compatibilidad con un DAW concreto, prueba el Sender y Receiver en ese entorno.
