...
& (Join-Path $PSScriptRoot 'sync-ui-theme.ps1') -Check
...
function Assert-InstallerBrandIcon {
  param(
    [Parameter(Mandatory)] [string]$InstallerPath
  )

  Add-Type -AssemblyName System.Drawing
  if ($null -eq ("DhreLinkInstallerIconNative" -as [type])) {
    Add-Type @'
using System;
using System.Runtime.InteropServices;

public static class DhreLinkInstallerIconNative
{
...
'@
  ...
  $brandGreen = 77
  $brandBlue = 250
  $iconHandles = [IntPtr[]]::new(1)
  $iconIds = [uint32[]]::new(1)
  $iconCount = [DhreLinkInstallerIconNative]::PrivateExtractIcons(
    $InstallerPath,
    0,
    $canvasSize,
    $canvasSize,
    ...
    1,
    0)
  if ($iconCount -ne 1 -or $iconHandles[0] -eq [IntPtr]::Zero) {
    throw "Could not extract the 48x48 icon from the installer: $InstallerPath"
  }

  $icon = $null
  $bitmap = $null
  try {
    $icon = [Drawing.Icon]::FromHandle($iconHandles[0])
    ...
    for ($y = 0; $y -lt $canvasSize; $y++) {
      for ($x = 0; $x -lt $canvasSize; $x++) {
        ...
        if ($pixel.A -ge 128) {
          ...
          if ([Math]::Abs($pixel.R - $brandRed) -le 8 -and
              [Math]::Abs($pixel.G - $brandGreen) -le 8 -and
              [Math]::Abs($pixel.B - $brandBlue) -le 8) {
            $brandPixels++
          }
        }
      }
    ...
    if ($coreWidth -lt 40 -or $coreHeight -lt 40 -or
        $brandPixels -lt 100 -or $transparentPixels -lt 100) {
      throw "Installer icon does not contain the purple dhreian mark: ${coreWidth}x${coreHeight}, $brandPixels brand pixels, $transparentPixels transparent pixels."
    }

    Write-Host "Installer brand icon verified: ${coreWidth}x${coreHeight}, $brandPixels purple pixels."
  }
  finally {
    if ($null -ne $bitmap) {
      $bitmap.Dispose()
    ...
    [DhreLinkInstallerIconNative]::DestroyIcon($iconHandles[0]) | Out-Null
  }
}
...
$stagedObsIcon = Join-Path $resolvedStageRoot 'obs\dhreLink\data\dhreian-mark-purple-512.png'
...
Assert-InstallerBrandIcon -InstallerPath $installer
